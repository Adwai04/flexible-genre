...
class PairedTransformerBinnedWeighted(nn.Module):
    def __init__(self,
                 n_bins: int,
                 num_inputs: int,
                 num_labels: int,
                 ...
                 dim_feedforward: int = 512,
                 dropout: float = 0.1):
        super(PairedTransformerBinnedWeighted, self).__init__()
        ...
        # +num_inputs because cost weights are concatenated with xf
        self.real_embed = RealEmbeddingNet(num_inputs * 2 + num_labels, emb_size)
        self.tgt_real_embed = RealEmbeddingNet(num_inputs + num_labels, emb_size)

        self.positional_encoding = LearnedPositionEncoding(
            emb_size, dropout=dropout, max_len=num_inputs * 2 + num_labels
        )
        self.tgt_positional_encoding = LearnedPositionEncoding(
        ...
        # Binning layer (shared for all positions)
        self.fc = nn.Linear(emb_size, n_bins)
        bins = torch.linspace(0, 1, n_bins + 1)[:-1]
        self.register_buffer("bins", bins)
    ...
    def forward(self,
        ...
        xf, xcf: input features (BxD)
        yf, ycf: binary labels (Bx1)
        weights: cost weights (BxD)
        """

        yf = 2 * yf - 1
        ycf = 2 * ycf - 1

        # Concatenate cost weights with features
        interleaved_features = torch.stack((xf, weights), dim=2) #[B, d] -> [B, d, 2]
        interleaved_features = interleaved_features.flatten(start_dim=1) #[B, d, 2] -> [B, 2*d]
        src = torch.cat((yf, interleaved_features), dim=1) #[B, 1] + [B, 2d] -> [B, 2d + 1]
        # src = torch.cat((yf, xf, weights), dim=1)      # [y, xf, weights]
        tgt = torch.cat((ycf, xcf), dim=1)    # [ycf, xcf, weights]

        src_emb = self.positional_encoding(self.real_embed(src))
        tgt_emb = self.tgt_positional_encoding(self.tgt_real_embed(tgt))
        ...
        bin_weights = (-(self.n_bins ** 2) * ((xcf[:, :, None] - self.bins.reshape(1, -1)) ** 2) / 2).softmax(dim=-1)
        loss = -(log_prob * bin_weights).sum(dim=-1)
    ...
    def _encode(self, xf: Tensor, yf: Tensor, weights: Tensor):
        yf = 2 * yf - 1
        interleaved_features = torch.stack((xf, weights), dim=2) #[B, d] -> [B, d, 2]
        interleaved_features = interleaved_features.flatten(start_dim=1) #[B, d, 2] -> [B, 2*d]
        src = torch.cat((yf, interleaved_features), dim=1) #[B, 1] + [B, 2d] -> [B, 2d + 1]
        # src = torch.cat((yf, xf, weights), dim=1)
        return self.transformer.encoder(self.positional_encoding(self.real_embed(src)))

    def _decode(self, hist: Tensor, tgt_mask: Tensor, y: Tensor, memory: Tensor, weights: Tensor):
        y = 2 * y - 1
        tgt = torch.cat((y, hist), dim=1)
        return self.fc(self.transformer.decoder(
            self.tgt_positional_encoding(self.tgt_real_embed(tgt)), memory, tgt_mask
        ))[:, -1]

    @torch.no_grad()
    def _sample(self, xf: Tensor, yf: Tensor, y: Tensor, weights: Tensor,
                sigma: float = 0.0, temp: float = 1.0):
        """
        Sampling with flexible cost conditioning.
        """
        self.eval()
        yf = yf.view(-1, 1)
        y = y.view(-1, 1)
        memory = self._encode(xf, yf, weights)
        batch_size = xf.shape[0]
        hist = torch.zeros((batch_size, 0), device=xf.device)

        for i in range(self.num_inputs):
            mask = self.generate_square_subsequent_mask(i + 1)
            logits = temp * self._decode(hist, mask, y, memory, weights)
            probs = logits.softmax(dim=-1)
            sampled_idx = torch.multinomial(probs, 1)
            bin_val = self.bins[sampled_idx]
            samp_val = bin_val + sigma * torch.randn_like(bin_val)
            hist = torch.cat((hist, samp_val), dim=1)
        return hist
...
def train_epoch(model, optimizer, pair_loader, epoch, DEVICE, show_bar=True):
    ...
    with tqdm(pair_loader, unit="batch", leave=show_bar) as tepoch:
        for i, batch in enumerate(tepoch):
            ...
            loss = model(xf, yf, xcf, ycf, weights).mean()
